...
#pragma prefast(suppress \
    ...
    char *data = reinterpret_cast<PCHAR>(pData);
    cout << lBytesWritten << " : " << NumFrames << endl;
    double avarageSample = 0;
    int iterations = 0;
    for (int i = 0; i < lBytesWritten; i += 8)
    {
        // frame between i and i+7
        // 1 channel between i and i+3
        // endianness might be wrong
        float sample = (float)(((data[i + 1] << 8) | (data[i] & 0xff)) / 32768.0);
        if (sample < 0)
        {
            sample *= -1;
        }
        avarageSample += sample;
        iterations++;
    }
    avarageSample /= iterations;
    std::cout << avarageSample << std::endl;
